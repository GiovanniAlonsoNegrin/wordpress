    ...
    class advancediFrame
    {
		...
		*/
		...
		function createMinimizedAiJs($backend) {
			...
			$url = 'http://www.toptal.com/developers/javascript-minifier//raw';     
		...
		function ai_getlatestVersion() {
			...
			if ($aip_version !== false) {
			   return $aip_version;
				...
				$data = http_build_query(array('url' => get_site_url(), 'codehash' => $rand . base64_encode($purchaseCode)   , 'sitehash' => hash('sha256', $auth_key), 'type' => $pro), '', '&'); 
